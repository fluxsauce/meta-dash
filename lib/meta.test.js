...
describe('./lib/meta.js', () => {
  ...
  test('returns a meta projects file if there are no problems', () => {
    fs.existsSync.mockReturnValueOnce(true);
    fs.readFileSync.mockImplementation(() => Buffer.from('{"projects":{"meta-dash":"git@github.com:fluxsauce/meta-dash.git"}}'));
    expect(meta.get())
      .toStrictEqual({
        projects: {
          'meta-dash': 'git@github.com:fluxsauce/meta-dash.git',
        },
      });
  });
});
